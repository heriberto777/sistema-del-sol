...
/** Historial cronológico con saldo corriente (Fase 5a) — molde calcado de LibroMayorView, pero fijo a una variante+bodega (sin selector: ya vienen resueltos por el caller, ver Inventario.tsx). */
export function KardexView({ varianteId, bodegaId }: { varianteId: string; bodegaId: string }) {
  const [desde, setDesde] = useState('');
  const [hasta, setHasta] = useState('');

  const { data, isLoading, error } = useQuery({
    queryKey: ['inventario-kardex', varianteId, bodegaId, desde, hasta],
    queryFn: async () =>
      (
        await apiClient.get<Kardex>(`/inventario/kardex/${varianteId}`, {
          params: { bodegaId, desde: desde || undefined, hasta: hasta || undefined },
        })
      ).data,
  });
