...
const PLANTILLAS_PEDIDO: { value: PlantillaPedidoTienda; glifo: string; label: string; descripcion: string }[] = [
  { value: 'RECIBO', glifo: '🧾', label: 'Recibo Térmico', descripcion: 'Estilo ticket de POS, monoespaciado' },
  { value: 'MARCA', glifo: '✓', label: 'Tarjeta de Marca', descripcion: 'Checkout moderno con tu color de acento' },
  { value: 'BOUTIQUE', glifo: '✦', label: 'Boutique Cálido', descripcion: 'Tono editorial, cercano' },
  { value: 'PANEL', glifo: '▤', label: 'Panel Compacto', descripcion: 'Tipo resumen/dashboard' },
];
...
const GRUPOS_SECCIONES: { grupo: string; items: { id: SeccionConfig; etiqueta: string; icono: LucideIcon }[] }[] = [
  {
    grupo: 'Tienda',
    items: [
      { id: 'identidad', etiqueta: 'Identidad y estado', icono: Store },
      { id: 'logo-banner', etiqueta: 'Logo y banner', icono: ImageIcon },
      { id: 'plantilla', etiqueta: 'Plantilla y color', icono: LayoutGrid },
      { id: 'catalogo', etiqueta: 'Catálogo', icono: Warehouse },
    ],
  },
  {
    grupo: 'Apariencia',
    items: [
      { id: 'apariencia', etiqueta: 'Apariencia', icono: Palette },
      { id: 'banner-anuncio', etiqueta: 'Banner de anuncio', icono: Megaphone },
      { id: 'ofertas', etiqueta: 'Ofertas e insignias', icono: Tag },
    ],
  },
  {
    grupo: 'Checkout',
    items: [{ id: 'confirmacion-pedido', etiqueta: 'Confirmación de pedido', icono: Receipt }],
  },
  {
    grupo: 'Estructura',
    items: [{ id: 'menu', etiqueta: 'Menú de navegación', icono: MenuIcon }],
  },
];
...
export function TiendaOnlineConfigPanel() {
  ...
  const [seccion, setSeccion] = useState<SeccionConfig>('identidad');
  ...
  useEffect(() => {
    ...
    setPlantillaPedido(
      PLANTILLAS_PEDIDO.some((p) => p.value === plantillaPedidoGuardada) ? (plantillaPedidoGuardada as PlantillaPedidoTienda) : 'MARCA',
    );
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-[220px_1fr]">
        <nav className="border-b border-slate-200 p-3 dark:border-slate-800 md:border-b-0 md:border-r">
          {GRUPOS_SECCIONES.map((grupo) => (
            <div key={grupo.grupo} className="mb-3 last:mb-0">
              <p className="px-2.5 pb-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">{grupo.grupo}</p>
                ...
                {grupo.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={clsx(
                        'flex items-center gap-2 rounded-lg border-l-2 px-2.5 py-2 text-left text-sm font-medium transition-colors',
                        seccion === item.id
        ...
        <div className="flex flex-col">
          <div className="flex-1 p-5">
              ...
              <div className="flex flex-col gap-4">
                {urlTienda && (
                  <div className="flex flex-col gap-1 rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800/50">
                    <span className="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">Enlace de tu tienda</span>
                    {activaGuardada ? (
                      <a
                        href={urlTienda}
                        target="_blank"
                      ...
                      <>
                        ...
                        <span className="text-xs text-amber-600 dark:text-amber-400">Marcá "Tienda activa" y guardá para que este enlace funcione.</span>
                      </>
                    )}
                    {activaGuardada &&
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400">
                    Usado por Directo/Mercado/Boutique. El resto de las plantillas usa la sección Apariencia.
                  </span>
                </div>
              </div>
            )}
            ...
            {seccion === 'apariencia' && (
              <div className="divide-y divide-slate-100 dark:divide-slate-800">
                <p className="pb-5 text-xs text-slate-500 dark:text-slate-400">
                  Se aplica sobre la plantilla elegida en "Plantilla y color". No tiene efecto en Directo/Mercado/Boutique, que siguen
                  usando el color de acento clásico de esa sección.
                </p>
                ...
                <SeccionPersonalizacion titulo="Tipografía">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <Select id="tema-tamano" value={tema.tamanoFuente} onChange={(e) => setTema({ ...tema, tamanoFuente: e.target.value as TemaTienda['tamanoFuente'] })}>
            ...
            {seccion === 'confirmacion-pedido' && (
              <div className="flex flex-col gap-4">
                <div>
                  <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Confirmación de pedido</h3>
                  <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    Qué ve el cliente al terminar de pagar — con el resumen de productos y el total ya calculado con ITBIS.
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {PLANTILLAS_PEDIDO.map((p) => (
                    <button
                      key={p.value}
                      type="button"
                      onClick={() => setPlantillaPedido(p.value)}
                      className={clsx(
                        'flex flex-col items-center gap-1 rounded-lg border px-3 py-3 text-center transition-colors',
                        plantillaPedido === p.value
                          ? 'border-sol-500 bg-sol-50 dark:bg-sol-500/10'
                          : 'border-slate-200 hover:border-slate-300 dark:border-slate-700 dark:hover:border-slate-600',
                      )}
                    >
                      <span className="text-xl">{p.glifo}</span>
                      <span
                        className={clsx(
                          'text-xs font-semibold',
                          plantillaPedido === p.value ? 'text-sol-700 dark:text-sol-300' : 'text-slate-700 dark:text-slate-300',
                        )}
                      >
                        {p.label}
                      </span>
                      <span className="text-[11px] text-slate-500 dark:text-slate-400">{p.descripcion}</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="flex justify-end border-t border-slate-200 bg-slate-50 px-5 py-3 dark:border-slate-800 dark:bg-slate-800/40">
