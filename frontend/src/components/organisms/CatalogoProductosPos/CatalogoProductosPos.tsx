 ...
 * tarjeta agrega 1 unidad directo al carrito, sin combobox+botón
 * separados. Usa `/productos/catalogo` (no `/productos`): ya trae la
 * imagen y el precio vigente en la misma respuesta, para no pedir el
 * precio aparte por cada producto como hacía la versión anterior.
 */
export function CatalogoProductosPos({ onAgregar }: { onAgregar: (producto: ProductoCatalogo) => void }) {
  const [busqueda, setBusqueda] = useState('');
  const busquedaDebounced = useDebouncedValue(busqueda);

  const { data, isLoading } = useQuery({
    queryKey: ['pos-catalogo', busquedaDebounced],
    queryFn: async () =>
      (
        await apiClient.get<PaginaResultado<ProductoCatalogo>>('/productos/catalogo', {
          params: { busqueda: busquedaDebounced || undefined, tamanoPagina: 24 },
        })
      ).data,
  });
