...
const ENLACES_MENU: Record<ClaveMenuTienda, { label: string; href: (subdominio: string) => string }> = {
  ...
  categorias: { label: 'Piezas', href: (s) => `/tienda/${s}/productos` },
...
function RoperoHome({ config, subdominio, carrito }: PropsHome) {
    ...
    <div className="min-h-screen bg-[var(--tienda-color-fondo)] text-[var(--tienda-color-texto)]" style={{ ...variablesCssTema(tema, DEFAULTS, modo), fontFamily: 'var(--tienda-fuente-body)', fontSize: 'var(--tienda-tamano-fuente)' }}>
      ...
      <div className="mx-auto grid max-w-4xl gap-10 px-6 py-12 sm:grid-cols-2 sm:px-10">
        ...
        <div>
          ...
          <button type="button" onClick={onAgregar} disabled={!varianteSeleccionada || (varianteSeleccionada.stock !== null && varianteSeleccionada.stock <= 0)} className="rounded px-6 py-3 text-[0.85em] font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50" style={{ background: 'var(--tienda-color-acento)' }}>
            Agregar a la bolsa
          </button>
