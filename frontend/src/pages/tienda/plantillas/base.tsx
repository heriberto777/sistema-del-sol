...
// Fase 8 — ropa básica de uso diario, insp. Uniqlo/Everlane: carmesí+malva
// sobre blanco, foco en tela/corte, sin urgencia de descuento.
const DEFAULTS: DefaultsTemaPlantilla = {
  colorAcento: '#d32820',
  colorFondo: '#ffffff',
  colorSuperficie: '#f4f2ee',
  colorTexto: '#1a1a1a',
...
function Nav({ nombre, logo, subdominio, cantidadCarrito, menu }: { nombre: string; logo: string | null; subdominio: string; cantidadCarrito: number; menu: ClaveMenuTienda[] }) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-y-2 px-6 py-5 sm:px-10">
      ...
      <div className="flex flex-wrap items-center gap-3 gap-y-2 sm:gap-5">
        {menu.map((clave) => {
          ...
          if (clave === 'carrito') {
            return (
              <button key={clave} type="button" onClick={abrir} className="flex items-center gap-2 rounded px-3 py-1.5 text-[0.8em] font-bold text-white" style={{ background: 'var(--tienda-color-acento)' }}>
...
function BaseHome({ config, subdominio, carrito }: PropsHome) {
  ...
  return (
    <div className="min-h-screen bg-[var(--tienda-color-fondo)] text-[var(--tienda-color-texto)]" style={{ ...variablesCssTema(tema, DEFAULTS, modo), fontFamily: 'var(--tienda-fuente-body)', fontSize: 'var(--tienda-tamano-fuente)' }}>
      ...
      <div className="max-w-md px-6 pb-6 pt-8 sm:px-10">
        <div className="mb-2 text-[0.7em] font-semibold uppercase tracking-[0.08em]" style={{ color: '#c46c7e' }}>
          Cápsula esencial
        </div>
...
function BaseProducto({ config, subdominio, carrito, producto, varianteSeleccionada, onSeleccionarVariante, cantidad, onCantidadChange, onAgregar }: PropsProducto) {
  const { tema, nombre, logo } = config;
  const { modo } = useTiendaTema();
  useCargarFuentesTienda([tema.fuenteDisplay ?? DEFAULTS.fuenteDisplay, tema.fuenteBody ?? DEFAULTS.fuenteBody]);
  const menu = menuVisibleOrdenado(tema.menu);
        ...
        <div>
          ...
          <div className="mb-4">
            {varianteSeleccionada ? (
              ...
              <p className="text-[1.2em] font-bold">Elegí una opción</p>
            )}
          </div>
          ...
          <BotonWhatsAppProducto
            ...
            className="mt-3 flex w-full items-center justify-center gap-2 rounded border border-[color:var(--tienda-color-texto)]/15 px-6 py-3 text-[0.85em] font-semibold"
          />
        </div>
      </div>
...
function BaseCarrito({ config, subdominio, carrito }: PropsCarrito) {
  const { tema, nombre, logo } = config;
  const { modo } = useTiendaTema();
  useCargarFuentesTienda([tema.fuenteDisplay ?? DEFAULTS.fuenteDisplay, tema.fuenteBody ?? DEFAULTS.fuenteBody]);
  const menu = menuVisibleOrdenado(tema.menu);
  return (
    <div className="min-h-screen bg-[var(--tienda-color-fondo)] text-[var(--tienda-color-texto)]" style={{ ...variablesCssTema(tema, DEFAULTS, modo), fontFamily: 'var(--tienda-fuente-body)', fontSize: 'var(--tienda-tamano-fuente)' }}>
      ...
      <div className="mx-auto max-w-2xl px-6 py-12 sm:px-10">
        <h1 className="mb-6 text-[1.3em] font-bold" style={{ fontFamily: 'var(--tienda-fuente-display)' }}>
          Tu bolsa
        </h1>
        ...
        {carrito.items.length > 0 && (
          <div className="mt-8 flex flex-col items-end gap-3 border-t border-[color:var(--tienda-color-texto)]/10 pt-6">
            <p className="text-[1.05em] font-bold">Total: {formatearPrecio(carrito.total)}</p>
...
export const base: Plantilla = { Home: BaseHome, Producto: BaseProducto, Carrito: BaseCarrito };
