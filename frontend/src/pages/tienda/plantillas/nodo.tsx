  ...
  return (
    <div className="flex items-center justify-between border-b border-[color:var(--tienda-color-texto)]/10 px-6 py-4 sm:px-10">
      <Link to={`/tienda/${subdominio}`} className="flex items-center gap-2" style={{ fontFamily: 'var(--tienda-fuente-display)' }}>
        {logo && <img src={logo} alt={nombre} className="h-8 w-8 rounded-md object-cover" />}
...
function NodoHome({ config, subdominio, carrito, productos, cargando, busqueda, onBuscar }: PropsHome) {
      ...
      <div id="catalogo" className="flex items-baseline justify-between px-6 pb-4 pt-4 sm:px-10">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Buscar…"
            className="rounded-lg border border-[color:var(--tienda-color-texto)]/15 bg-[var(--tienda-color-superficie)] py-2 pl-8 pr-3 text-[0.8em] outline-none"
          />
        </div>
      </div>

      <div className="grid grid-cols-2 gap-3 px-6 pb-16 sm:grid-cols-3 sm:px-10 lg:grid-cols-4">
        ...
        {productos.map((p) => (
          <Link key={p.id} to={`/tienda/${subdominio}/producto/${p.id}`} className="overflow-hidden rounded-[var(--tienda-radio-tarjeta)] border border-[color:var(--tienda-color-texto)]/10 bg-[var(--tienda-color-superficie)] shadow-[var(--tienda-sombra-tarjeta)]">
            <ThumbNodo imagen={p.imagen} nombre={p.nombre} />
            <div className="p-3">
              {p.categoria && <div className="mb-1 text-[0.65em] uppercase tracking-wide opacity-40">{p.categoria.nombre}</div>}
              <h3 className="mb-1.5 text-[0.85em] font-semibold">{p.nombre}</h3>
    ...
    <div className="min-h-screen bg-[var(--tienda-color-fondo)] text-[var(--tienda-color-texto)]" style={{ ...variablesCssTema(tema, DEFAULTS), fontFamily: 'var(--tienda-fuente-body)', fontSize: 'var(--tienda-tamano-fuente)' }}>
      ...
      <div className="mx-auto grid max-w-4xl gap-10 px-6 py-12 sm:grid-cols-2 sm:px-10">
        ...
        <div>
          ...
          <button type="button" onClick={onAgregar} disabled={!varianteSeleccionada} className="rounded-lg px-6 py-3 text-[0.85em] font-bold text-white disabled:cursor-not-allowed disabled:opacity-50" style={{ background: 'var(--tienda-color-acento)' }}>
            Agregar al carrito
          </button>
        </div>
      </div>
