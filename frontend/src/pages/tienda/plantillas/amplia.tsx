...
// Fase 8 — moda talla grande/inclusiva, insp. Universal Standard: cálida,
// redondeada, tipografía amigable — mensaje de fit explícito en la ficha.
const DEFAULTS: DefaultsTemaPlantilla = {
  colorAcento: '#c1673f',
  colorFondo: '#faf3ec',
  colorSuperficie: '#ffffff',
  colorTexto: '#2e2621',
  fuenteDisplay: 'Poppins',
...
const ENLACES_MENU: Record<ClaveMenuTienda, { label: string; href: (subdominio: string) => string }> = {
  ...
  categorias: { label: 'Catálogo', href: (s) => `/tienda/${s}#catalogo` },
  ...
  return (
    <div className="flex items-center justify-between px-6 py-4 sm:px-10">
      <Link to={`/tienda/${subdominio}`} className="flex items-center gap-2" style={{ fontFamily: 'var(--tienda-fuente-display)' }}>
        ...
        <span className="text-[1.1em] font-semibold" style={{ color: 'var(--tienda-color-acento)' }}>
          {nombre}
        </span>
      </Link>
      <div className="flex items-center gap-5">
        {menu.map((clave) => {
          ...
          if (clave === 'carrito') {
            return (
              <button key={clave} type="button" onClick={abrir} className="flex items-center gap-2 rounded-full px-4 py-2 text-[0.8em] font-semibold text-white" style={{ background: '#8a8f5c' }}>
...
function ThumbAmplia({ imagen, nombre }: { imagen: string | null; nombre: string }) {
  return (
    <div
      className="w-full overflow-hidden rounded-[var(--tienda-radio-tarjeta)]"
      style={{ aspectRatio: 'var(--tienda-ratio-imagen)', background: 'linear-gradient(160deg, color-mix(in srgb, var(--tienda-color-acento) 25%, var(--tienda-color-superficie)), color-mix(in srgb, var(--tienda-color-acento) 12%, var(--tienda-color-superficie)))' }}
    >
...
function AmpliaHome({ config, subdominio, carrito, productos, cargando, busqueda, onBuscar }: PropsHome) {
  ...
  return (
    <div className="min-h-screen bg-[var(--tienda-color-fondo)] text-[var(--tienda-color-texto)]" style={{ ...variablesCssTema(tema, DEFAULTS), fontFamily: 'var(--tienda-fuente-body)', fontSize: 'var(--tienda-tamano-fuente)' }}>
      ...
      <div className="max-w-md px-6 pb-6 pt-8 sm:px-10">
        <div className="mb-2 text-[0.7em] font-semibold uppercase tracking-[0.05em]" style={{ color: '#8a8f5c' }}>
          Todas las tallas, siempre
        </div>
        <h1 className="mb-3 text-[1.7em] font-semibold" style={{ fontFamily: 'var(--tienda-fuente-display)' }}>
          {nombre}
        </h1>
        <p className="text-[0.85em] opacity-60">Ropa que se adapta a vos — de la XS a la 4X, con la misma atención al detalle.</p>
      </div>
      ...
      <div id="catalogo" className="flex items-baseline justify-between px-6 pb-4 pt-4 sm:px-10">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Buscar…"
            className="rounded-full border border-[color:var(--tienda-color-texto)]/15 bg-[var(--tienda-color-superficie)] py-2 pl-8 pr-3 text-[0.8em] outline-none"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 px-6 pb-16 sm:grid-cols-3 sm:px-10 lg:grid-cols-4">
        ...
        {productos.map((p) => (
          <Link key={p.id} to={`/tienda/${subdominio}/producto/${p.id}`} className="overflow-hidden rounded-[var(--tienda-radio-tarjeta)] bg-[var(--tienda-color-superficie)] shadow-[var(--tienda-sombra-tarjeta)]">
            <ThumbAmplia imagen={p.imagen} nombre={p.nombre} />
            <div className="p-3">
              <span className="mb-1 inline-block rounded-full px-2 py-0.5 text-[0.62em] font-semibold" style={{ background: 'color-mix(in srgb, #8a8f5c 18%, var(--tienda-color-superficie))', color: '#5f6440' }}>
                XS – 4X
              </span>
              <h3 className="mb-1 text-[0.85em] font-semibold">{p.nombre}</h3>
              <div className="flex items-center justify-between">
                <span className="text-[0.82em] font-bold" style={{ color: 'var(--tienda-color-acento)' }}>
...
function AmpliaProducto({ config, subdominio, carrito, producto, varianteSeleccionada, onSeleccionarVariante, cantidad, onCantidadChange, onAgregar }: PropsProducto) {
  const { tema, nombre, logo } = config;
  useCargarFuentesTienda([tema.fuenteDisplay ?? DEFAULTS.fuenteDisplay, tema.fuenteBody ?? DEFAULTS.fuenteBody]);
  const menu = menuVisibleOrdenado(tema.menu);
      ...
      <div className="mx-auto grid max-w-4xl gap-10 px-6 py-12 sm:grid-cols-2 sm:px-10">
        <div>
          <div className="overflow-hidden rounded-[var(--tienda-radio-tarjeta)]" style={{ aspectRatio: 'var(--tienda-ratio-imagen)' }}>
            <ThumbAmplia imagen={imagenActiva} nombre={producto.nombre} />
          </div>
          {galeria.length > 1 && (
...
function AmpliaCarrito({ config, subdominio, carrito }: PropsCarrito) {
  const { tema, nombre, logo } = config;
  useCargarFuentesTienda([tema.fuenteDisplay ?? DEFAULTS.fuenteDisplay, tema.fuenteBody ?? DEFAULTS.fuenteBody]);
  const menu = menuVisibleOrdenado(tema.menu);
  return (
      ...
      <div className="mx-auto max-w-2xl px-6 py-12 sm:px-10">
        ...
        <div className="flex flex-col gap-4">
          {carrito.items.map((item) => (
            <div key={item.varianteId} className="flex items-center gap-4 rounded-[var(--tienda-radio-tarjeta)] bg-[var(--tienda-color-superficie)] p-3 shadow-[var(--tienda-sombra-tarjeta)]">
              <div className="h-16 w-16 shrink-0 overflow-hidden rounded-2xl" style={{ background: 'color-mix(in srgb, var(--tienda-color-acento) 20%, var(--tienda-color-superficie))' }}>
        ...
        {carrito.items.length > 0 && (
          <div className="mt-8 flex flex-col items-end gap-3 border-t border-[color:var(--tienda-color-texto)]/10 pt-6">
            <p className="text-[1.1em] font-semibold" style={{ fontFamily: 'var(--tienda-fuente-display)' }}>
              Total: {formatearPrecio(carrito.total)}
            </p>
            <Link to={`/tienda/${subdominio}/checkout`} className="rounded-full px-6 py-3 text-[0.85em] font-semibold text-white" style={{ background: 'var(--tienda-color-acento)' }}>
              Finalizar compra
            </Link>
          </div>
        )}
...
export const amplia: Plantilla = { Home: AmpliaHome, Producto: AmpliaProducto, Carrito: AmpliaCarrito };
