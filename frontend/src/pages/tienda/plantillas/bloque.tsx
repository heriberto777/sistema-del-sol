...
// acento ácido. Aprobada en mockup previo a esta implementación.
const DEFAULTS: DefaultsTemaPlantilla = {
  colorAcento: '#d7ff3f',
  colorFondo: '#151414',
  colorSuperficie: '#1f1d1c',
  colorTexto: '#f2efe9',
  fuenteDisplay: 'Anton',
  fuenteBody: 'Barlow Condensed',
};

const ENLACES_MENU: Record<ClaveMenuTienda, { label: string; href: (subdominio: string) => string }> = {
  inicio: { label: 'Nuevo', href: (s) => `/tienda/${s}` },
  categorias: { label: 'Drops', href: (s) => `/tienda/${s}#catalogo` },
    ...
    <div className="flex items-center justify-between border-b border-[color:var(--tienda-color-texto)]/10 px-6 py-5 sm:px-10">
      ...
      <div className="flex items-center gap-6">
        {menu.map((clave) => {
          if (clave === 'cuenta') {
            return (
              <Link
                ...
                className="flex items-center gap-1.5 text-[0.78em] font-semibold uppercase tracking-[0.04em] text-[var(--tienda-color-texto)] opacity-80"
              >
                <User size={15} />
          ...
          if (clave === 'carrito') {
            return (
              <button
                ...
                type="button"
                onClick={abrir}
                className="flex items-center gap-2 px-4 py-2 text-[0.78em] font-bold uppercase tracking-[0.04em]"
          ...
          return (
            <Link key={clave} to={ENLACES_MENU[clave].href(subdominio)} className="text-[0.78em] font-semibold uppercase tracking-[0.04em] text-[var(--tienda-color-texto)] opacity-80">
...
function Footer({ nombre }: { nombre: string }) {
  return (
    <div className="flex justify-between border-t border-[color:var(--tienda-color-texto)]/10 px-6 py-6 text-[0.75em] uppercase tracking-[0.04em] opacity-50 sm:px-10">
      <span>© {nombre}</span>
...
function ThumbBloque({ imagen, nombre }: { imagen: string | null; nombre: string }) {
  return (
    <div
      className="w-full overflow-hidden rounded-[var(--tienda-radio-tarjeta)]"
      style={{ aspectRatio: 'var(--tienda-ratio-imagen)', background: 'linear-gradient(150deg, color-mix(in srgb, var(--tienda-color-texto) 12%, var(--tienda-color-fondo)), var(--tienda-color-fondo))' }}
    >
...
function BloqueHome({ config, subdominio, carrito, productos, cargando, busqueda, onBuscar }: PropsHome) {
  ...
  return (
    ...
    >
      ...
      <div id="catalogo" className="flex items-baseline justify-between px-6 pb-4 pt-4 sm:px-10">
        <h2 className="text-[1.05em] font-bold uppercase tracking-[0.03em]" style={{ fontFamily: 'var(--tienda-fuente-display)' }}>
          Catálogo
        </h2>
        <div className="relative">
          ...
          <input
            ...
            placeholder="Buscar…"
            className="border border-[color:var(--tienda-color-texto)]/15 bg-[var(--tienda-color-superficie)] py-2 pl-8 pr-3 text-[0.8em] outline-none"
          />
        </div>
      </div>

      <div className="grid grid-cols-2 gap-px bg-[color:var(--tienda-color-texto)]/10 px-6 pb-16 sm:grid-cols-3 sm:px-10 lg:grid-cols-4">
        {cargando && <p className="col-span-full bg-[var(--tienda-color-fondo)] py-6 text-[0.85em] opacity-60">Cargando…</p>}
        {!cargando && productos.length === 0 && <p className="col-span-full bg-[var(--tienda-color-fondo)] py-6 text-[0.85em] opacity-60">No hay productos.</p>}
        {productos.map((p) => (
          <Link key={p.id} to={`/tienda/${subdominio}/producto/${p.id}`} className="bg-[var(--tienda-color-fondo)] pb-3">
            <ThumbBloque imagen={p.imagen} nombre={p.nombre} />
            <div className="px-2 pt-2.5">
              <h3 className="mb-1 text-[0.9em] uppercase" style={{ fontFamily: 'var(--tienda-fuente-display)' }}>
                {p.nombre}
              </h3>
              <div className="flex items-center justify-between">
                <span className="text-[0.82em] font-semibold" style={{ color: 'var(--tienda-color-acento)' }}>
                ...
                {!p.tieneVariantes && p.varianteId && (
                  <button
                    ...
                    className="flex h-6 w-6 items-center justify-center rounded-[var(--tienda-radio-tarjeta)]"
                    style={{ background: 'var(--tienda-color-acento)', color: '#151414' }}
                  >
                    <Plus size={13} />
                  </button>
      ...
      <div className="mx-auto grid max-w-4xl gap-10 px-6 py-12 sm:grid-cols-2 sm:px-10">
        <div>
          <div className="overflow-hidden rounded-[var(--tienda-radio-tarjeta)]" style={{ aspectRatio: 'var(--tienda-ratio-imagen)' }}>
            <ThumbBloque imagen={imagenActiva} nombre={producto.nombre} />
          </div>
          {galeria.length > 1 && (
            <div className="mt-3 flex gap-2">
              {galeria.map((img, i) => (
                <button
                  ...
                  className="h-14 w-14 overflow-hidden border-2"
        ...
        <div>
          <h1 className="mb-2 text-[1.9em] uppercase leading-none" style={{ fontFamily: 'var(--tienda-fuente-display)' }}>
            {producto.nombre}
          </h1>
          <p className="mb-4 text-[1.3em] font-bold" style={{ color: 'var(--tienda-color-acento)' }}>
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto max-w-2xl px-6 py-12 sm:px-10">
        <h1 className="mb-6 text-[1.6em] uppercase" style={{ fontFamily: 'var(--tienda-fuente-display)' }}>
          Tu bolsa
        </h1>
        {carrito.items.length === 0 && <p className="text-[0.85em] opacity-60">Tu bolsa está vacía.</p>}
        <div className="flex flex-col gap-4">
          {carrito.items.map((item) => (
            <div key={item.varianteId} className="flex items-center gap-4 bg-[var(--tienda-color-superficie)] p-3">
              <div className="h-16 w-16 shrink-0 overflow-hidden rounded-[var(--tienda-radio-tarjeta)] bg-[var(--tienda-color-fondo)]">
                {item.imagen && <img src={item.imagen} alt={item.nombre} className="h-full w-full object-cover" />}
              </div>
              <div className="flex-1">
                <p className="text-[0.85em] font-semibold uppercase">{item.nombre}</p>
        ...
        {carrito.items.length > 0 && (
          <div className="mt-8 flex flex-col items-end gap-3 border-t border-[color:var(--tienda-color-texto)]/10 pt-6">
            <p className="text-[1.1em] font-bold uppercase" style={{ fontFamily: 'var(--tienda-fuente-display)' }}>
              Total: {formatearPrecio(carrito.total)}
            </p>
            <Link
              to={`/tienda/${subdominio}/checkout`}
              className="px-6 py-3 text-[0.85em] font-bold uppercase tracking-[0.03em]"
              style={{ background: 'var(--tienda-color-acento)', color: '#151414' }}
            >
              Finalizar compra
            </Link>
...
export const bloque: Plantilla = { Home: BloqueHome, Producto: BloqueProducto, Carrito: BloqueCarrito };
